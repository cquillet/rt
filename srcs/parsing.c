...
/*   Updated: 2018/04/23 20:33:06 by cquillet         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	parse_values(t_b *b, char **tab, size_t n)
{
	...
	if (n == 6 && !ft_strcmp(tab[0], "plane"))
		obj = add_obj(b, init_plane(init_vect(
			parse_double(tab[1]), parse_double(tab[2]), parse_double(tab[3])),
									parse_double(tab[4]), parse_col(tab[5])));
	else if (n == 4 && !ft_strcmp(tab[0], "sph"))
		obj = add_obj(b, init_sph(parse_vect(tab[1]), parse_col(tab[2]),
			parse_double(tab[3])));
	else if (n == 5 && !ft_strcmp(tab[0], "cyl"))
		obj = add_obj(b, init_cyl(parse_vect(tab[1]), parse_col(tab[2]),
									parse_vect(tab[3]), parse_double(tab[4])));
	else if (n == 5 && !ft_strcmp(tab[0], "cone"))
		obj = add_obj(b, init_cone(parse_vect(tab[1]), parse_col(tab[2]),
									parse_vect(tab[3]), parse_double(tab[4])));
	else
		return (-1);
